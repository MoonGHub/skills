...
- **Scope and mode**: Initialize/Refresh/Deepen matches the request; Refresh repaired confirmed gaps incrementally, while Deepen evaluated bounded meaningful descendants without leaving the requested area unless an explicit full-audit trigger existed.
- **Routing and semantic depth**: START_HERE task rows and the mode-specific bounded coverage candidates reach or explicitly route every meaningful owner and high-risk boundary in scope to its local role, risks, adjacent contracts, and validation gate; ancestor-file presence or a container-only review was not accepted, while transparent directories and ordinary leaves add no guide noise.
...
- **Context efficiency**: rules and facts have one owner, folder AGENTS contain only actionable local delta and respect the soft budget or state a preservation reason, and semantic depth improves routing without mirroring the directory tree or adding navigation.
...
Report generated/updated/retired/preserved paths, remaining `확인 필요`, validation performed, and why application tests were skipped when documentation alone changed. For Deepen, also report each included meaningful descendant's local-guide or actionable-ancestor decision and why a requested container guide was intentionally omitted.
